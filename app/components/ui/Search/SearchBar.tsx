...
import { ChangeEventHandler, FC, InputHTMLAttributes, SyntheticEvent, useState } from 'react'
    ...
    return (
        <>
            ...
            >
                <div
                    className={cn(
                        'border', 'border-blue-500', 'rounded', 'h-[52px]', 'flex', 'items-center', 'w-10/12'
                    )}
                >
                    <div className={cn(
                        'm-3'
                    ...
                    <input placeholder={placeholder} type='search' className={cn(
                        'border-none', 'w-full', 'text-xl', 'h-full', 'pb-4',
                        'text-blue-500', 'active:outline-none', 'focus:outline-none',
                ...
                <div className={cn('p-3', 'relative')}>
                    ...
                    <div className={cn('absolute', 'bg-white', 'border', 'border-blue-500', 'py-4',
                        ...
                        'rounded-3xl', 'z-1', 'right-[50px]', sortVisible ? 'visible' : 'hidden')}>
                        {
                            filters.map((filt) => {
                                return (<>
                                    <div className={cn('flex', 'flex-row', 'gap-4', 'items-center', 'text-xl')}>
                                        <input className={cn('w-6', 'h-6')} type='checkbox' checked={filt.isVisible} onChange={checkboxChange} name={filt.name}></input>
                                        <p> {filt.name}</p>
                                    </div>
                                </>)
